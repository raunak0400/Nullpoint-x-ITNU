...
import { useSharedState, locations, Location } from './sidebar';
...
export function Header() {
  ...
  const pageTitles: { [key: string]: { title: string, description: (loc: Location | null) => string } } = {
    '/': { title: selectedLocation ? `${selectedLocation.name}, ${selectedLocation.country}` : 'Dashboard', description: () => '' },
    '/weather-info': { title: 'Weather Info', description: (loc) => loc ? `Detailed view for ${loc.name}, ${loc.country}` : 'Search for a location to see details' },
    '/future-air-prediction': { title: 'Future Air Prediction', description: (loc) => loc ? `AI-powered 24-hour AQI forecast for ${loc.name}` : 'Search for a location for a forecast' },
    '/air-alerts': { title: 'Air Alerts', description: (loc) => loc ? `Current advisories for ${loc.name}` : 'Search for a location to see alerts' },
    '/map-view': { title: 'Interactive Map View', description: (loc) => loc ? `Data layers for ${loc.name}`: 'Global data layers' },
    '/past-air-data': { title: 'Past Air Data', description: (loc) => loc ? `Historical pollutant levels in ${loc.name}` : 'Search for a location to see past data' },
  };
  ...
  const currentPage = pageTitles[pathname] || { title: 'Dashboard', description: () => 'Real-time air quality monitoring' };
    ...
    <header className="flex flex-wrap items-center justify-between gap-4">
      <div>
        ...
        <p className="text-muted-foreground flex items-center" suppressHydrationWarning>
          ...
          ) : currentPage.description(selectedLocation) || 'Search for a location to begin.'}
        </p>
      </div>
