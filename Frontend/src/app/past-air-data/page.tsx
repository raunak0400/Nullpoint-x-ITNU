...
export default function PastAirDataPage() {
  ...
  useEffect(() => {
    if (selectedLocation && date?.from && date?.to) {
      // API call to fetch historical data would go here
      // const fetchedData = await fetchHistoricalData(date, selectedLocation);
      // setData(fetchedData);
      setData([]);
    } else {
      setData([]);
            ...
            <div className="h-96">
              {!selectedLocation ? (
                <div className="flex h-full items-center justify-center text-muted-foreground">
                  Please select a location to see historical data.
                </div>
              ) : data.length > 0 ? (
              <ResponsiveContainer width="100%" height="100%">
                ...
                <div className="flex h-full items-center justify-center text-muted-foreground">
                  Select a date range to see historical data for {selectedLocation.name}.
                </div>
              )}
            </div>
          </CardContent>
